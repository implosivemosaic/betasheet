...
- `Catalogue.Listing` — Ecto schema and changeset. This is the destination format: kind, label, summary, schedule shape and dates, price state and note, audience and ages, one link and its kind, optional offsite venue, confidence, caveat, sources, checked date, published. Closed vocabularies are also CHECK constraints in the migration. `Catalogue.Occurrence` holds confirmed dates. `Catalogue.Venue` is one row per research gym.
...
- `Format` — pure date/time/distance/label formatting; `price/1` renders from `price_state` + `price_note` only.
